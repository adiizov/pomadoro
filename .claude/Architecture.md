...
- [x] Шаг 4 — Tauri (desktop): `src-tauri` (Tauri 2), плагины store/notification/single-instance, tray-меню + close-to-tray, mini-mode (`set_mini` + `platform.window`), адаптер `platform/tauri`, runtime-детект, vite под tauri. **Rust компилируется, `tauri:build` собирает Linux-инсталляторы** (AppImage/.deb/.rpm в `src-tauri/target/release/bundle/`). Системные библиотеки установлены (§9). Runtime tray/mini-mode ещё не гоняли в GUI. Auto-updater отложен на шаг 9 (ключи подписи + GitHub Releases).
